...
void metropolisJoint()
{
    ...
    for(nf=0;nf<NFil;nf++)
    {
        ...
        tBase[nf][1]=0;
        tBase[nf][2]=1;
        rBase[nf][0]=baseSepDistance * cos( (double) nf*(2*PI/NFil));
        rBase[nf][1]=baseSepDistance * sin( (double) nf*(2*PI/NFil));
        rBase[nf][2]=0;
        e1Base[nf][0]=1;
        e1Base[nf][1]=0;
        e1Base[nf][2]=0;
    ...
    // General: make sure these will not interact in initial condition (if they do then only moving one filament will probably never be enough to find an acceptable configuration)
    for(nf=0;nf<NFil;nf++)
    {
